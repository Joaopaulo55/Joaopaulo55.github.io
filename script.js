...
let lastPopupTime = 0;
const POPUP_COOLDOWN = 30000; // 30 segundos entre popups
...
function initEventSources() {
  ...
  eventSource.onmessage = (e) => {
    try {
      ...
      // Verifica se passou tempo suficiente desde o último popup
      const now = Date.now();
  ...
  eventSource.onerror = (e) => {
    ...
    setTimeout(initEventSources, 10000); // Tenta reconectar após 10s
  };
}
...
function showNewsPopup(randomNumber) {
  ...
  // Fecha o popup se já estiver aberto
  newsPopup.classList.add('hidden');
  
  // Espera um pouco antes de mostrar o novo popup
  setTimeout(() => {
    ...
    newsPopup.style.animation = 'fadeIn 0.3s ease-in-out';
    
    // Fecha automaticamente após 10 segundos
    setTimeout(() => {
      if (!newsPopup.classList.contains('hidden')) {
        newsPopup.style.animation = 'fadeOut 0.3s ease-in-out';
        setTimeout(() => newsPopup.classList.add('hidden'), 300);
      }
    }, 10000);
  }, 300);
}
...
function updateVideoInfoUI(data) {
  ...
  if (formats && data.formats && data.formats.length > 0) {
    ...
    formats.innerHTML = sortedFormats.map(f => {
      ...
      return `<option value="${f.format_id || f.format}">
...
async function downloadVideo() {
  ...
  try {
    ...
    const data = await response.json();
    handleDownload(data.downloadUrl || data.download);
    simulateProgress();
